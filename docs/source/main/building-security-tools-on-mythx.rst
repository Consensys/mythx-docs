...
The source list and `sources` dict must contain all files necessary for compiling the contract - i.e. the main Solidity file as well as imports (and imports of imports, etc.). Each filename listed in `sourceList` must have a matching key in the `sources` dict. Note that the order of the files in the `sourceList` field is crucial for receiving correct issue locations and should match the order that the compiler used to build the source maps.
